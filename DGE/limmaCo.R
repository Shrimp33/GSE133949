...
counts$Symbol <- NULL
counts$id <- NULL
colnames(counts) <- c("t1", "c1", "c2", "c3", "c4", "c5", "c6", "t2", "t3", "t4", "t5", "t6", "t7")
...
saveRDS(d, "Colon_counts_pass_cutoff.rds")
...
#     TRUE ~ "Downregulated"
#   )) %>%
#   mutate(Rank = 1:n(), Label = ifelse(((Rank < topN) & adj.P.Val < p_cutoff), row.names(.),"")) %>%
#   ggplot(aes(x = logFC, y = -log10(adj.P.Val), color = ColorCategory, label = Label)) +
#   geom_point() +
#   geom_text_repel(col = "black", box.padding = 0.5, max.overlaps = Inf, size=12) +  # Increase max.overlaps
...
# write.csv(h_sig, "h_sig_colon.csv")
...
stori <- c(u$Symbol[1:15], d$Symbol[1:10], c("Fgfbp1"))
...
write.csv(full_results, "full_results_colon.csv")

stori <- stori[!stori %in% c("Vstm5", "Gdpd5", "Fhdc1", "Atp2b1")]
...
p <- full_results %>%
  ...
  ggplot(aes(x = logFC, y = -log10(adj.P.Val), color = ColorCategory, label = Label)) +
  geom_point() +
  geom_text_repel(aes(size = ifelse(Label %in% c("Fgfbp1"), 24/.pt, 14/.pt)),
                  col = "black", box.padding = 1.5, max.overlaps = Inf, min.segment.length = unit(0, 'lines'), force = 2) +  
...
plot <- p + scale_x_break(c(5, 10), scales=0.2, expand = expansion(add=0.5), ticklabels = c(10, 10.5))

ggsave("full_results_colon_scaled.png", plot, width = 9.38, height = 7.5, units = "in", dpi = 300)
